...
Windows services are controlled by the Service Control Manager (SCM), a system process responsible for starting, stopping, and monitoring services. The SCM keeps track of each service’s current state and provides the mechanisms needed to configure them.

Every service on a Windows system is linked to a specific executable. When a service is started, the SCM launches this executable. These executables must implement special service-related functions so they can properly communicate with the SCM, which means a regular program cannot simply be run as a service. Additionally, each service defines the user account under which it operates.

To get a clearer picture of how a service is structured, you can examine the configuration of the **apphostsvc** service using the `sc qc` command:
...
![img](/img/WindowsPrivEsc/1.png)

All of the services configurations are stored on the registry under `HKLM\SYSTEM\CurrentControlSet\Services\`:

![img](/img/WindowsPrivEsc/2.png)

A subkey exists for every service in the system. Again, we can see the associated executable on the **ImagePath** value and the account used to start the service on the **ObjectName** value. If a DACL has been configured for the service, it will be stored in a subkey called Security. As you have guessed by now, only administrators can modify such registry entries by default.

---

### 4.1 **Insecure Permissions on Service Executable**
...
After creating a payload we can transfer it on the windows machine and start a listener to receive the reverse shell when it gets executed:
...
### 4.3 **Insecure Service Permissions**

Even if a service’s executable permissions are properly secured and its binary path is correctly quoted, there may still be an opportunity to exploit the service. If the service’s own DACL (not the executable’s DACL) grants you permission to modify its configuration, you can change how the service is set up. This lets you redirect the service to run any executable you choose and execute it under any account specified for the service, including the SYSTEM account.

To check for a service DACL from the command line, you can use Accesschk from the Sysinternals suite. The command to check for the thmservice service DACL is:
...
We will then transfer the reverse shell executable to the target machine and move it to the desired location. Remember to grant permissions to Everyone to execute your payload:
...
**“Disabled” doesn’t mean unusable.**
`SeTakeOwnershipPrivilege` is **present in an admin token but off by default**.
Tools like `takeown.exe` **temporarily enable it at runtime** using `AdjustTokenPrivileges()`, do the job, then exit.

So:

* `cmd.exe` → privilege **disabled**
* `takeown.exe` → privilege **enabled just-in-time**
* Process ends → privilege state disappears

It’s a **safety catch**, not a lock.
...
### 5.3 **SeImpersonate / SeAssignPrimaryToken**
...
In this scenario the target server is running Druva inSync 6.6.3, which is vulnerable to privilege escalation as reported by [Matteo Malvica](https://www.matteomalvica.com/blog/2020/05/21/lpe-path-traversal/). The vulnerability results from a bad patch applied over another vulnerability reported initially for version 6.5.0 by Chris Lyne.
...
Initially published by [Matteo Malvica](https://packetstormsecurity.com/files/160404/Druva-inSync-Windows-Client-6.6.3-Privilege-Escalation.html), the following exploit can be used in your target machine to elevate privileges. Here is the original exploit's code:
...
Downloaded [WinPEAS](https://github.com/carlospolop/PEASS-ng/tree/master/winPEAS).

### 7.2 **PrivescCheck**

PrivescCheck is a PowerShell script that searches common privilege escalation on the target system. It provides an alternative to WinPEAS without requiring the execution of a binary file.

Downloaded [PrivescCheck](https://github.com/itm4n/PrivescCheck).
...
### 7.3 **WES-NG: Windows Exploit Suggester - Next Generation**
